...
btn.addEventListener('click', (e) => {
  ...
  if (isHidden) {
    // 1. Show the element (remove hidden)
    menu.classList.remove('hidden');
    
    // 2. Trigger reflow to let transition work, then remove scale/opacity transition classes
...
function closeMenu() {
  // 1. Transition back to closed state
  menu.classList.remove('opacity-100', 'scale-100');
  menu.classList.add('opacity-0', 'scale-95');
  
  // 2. Hide completely after animation finishes
  setTimeout(() => {
    menu.classList.add('hidden');
  }, 100); // matching duration-100
}
